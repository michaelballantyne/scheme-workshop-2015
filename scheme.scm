...
(define list-of-bindingso
  (lambda (bindings)
    (conde
      [(== '() bindings)]
      [(fresh (name x* body rest)
         (== `((,name . (lambda ,x* ,body)) . ,rest) bindings)
         (list-of-symbolso x*))])))
...
(define eval-expo
  (lambda (exp env val)
    (conde
      ...
      ((fresh (bindings letrec-body)
         (== `(letrec ,bindings ;; multi-function multiple-argument letrec version
                ,letrec-body)
             exp)
         ;(list-of-bindingso bindings)
         (not-in-envo 'letrec env)
         (eval-expo letrec-body
                    `(ext-rec ,bindings ,env)
                    val)))
